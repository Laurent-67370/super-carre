...
### ⚡ v88 — mascotte 3D allégée (−60% de nœuds DOM)

Optimisation de l'extrusion v87 : les couches clonées embarquaient tous les accessoires du catalogue masqués en `display:none` (chapeaux, costumes, lunettes non équipés) — soit 165 nœuds SVG × 7 copies = 1 155 éléments DOM pour la seule mascotte. Les nœuds invisibles sont désormais retirés des clones à la construction : seuls les éléments réellement affichés sont extrudés, soit environ 60 % d'éléments en moins, sans aucun changement visuel. Le volume se reconstruisant à chaque changement de skin, l'optimisation reste valable quelle que soit la tenue équipée.
